...
const char ap_ssid[] = "LS20-AP0026";
const char ap_pass[] = "Q6bZwk8z5K"; 
...
float ReadVoltage(byte ADC_PIN) {
  ...
  return ((analogRead(ADC_PIN) + 285)/ 4095.0) * 1.270 * (1100 / vref) * calibration;  // ESP by design reference voltage in mV
}
...
void loop()
{
 ...
 // Offset 10 10mv          Scale 35 10mv@ 800
  int out_duty1 = (in_volt1 + shift_value - 70) * 255 / 3110;
  int out_duty2 = (in_volt2 + shift_value - 60) * 255 / 3090;
 //                                         ^           ^
  if (af_value == "0.01")
  {
    out_duty1 = 31;
